...
- **默认（云端 API）**：`python3 skills/streamy-content-gen/scripts/query_market_facts.py --sources market,news,social --summary-only`（HTTP 拉云端 pre-Router → 本地 Router/Rewriter；须 `.env` 配 `FINANCE_CLOUD_API_*`）。**默认 cache-first**：新鲜 cache 直接返回，`meta.snapshot_cached=true`；仅 stale / `--force-refresh` 才云拉。成功时写 **`cache/snapshot/snapshot.json`**（与 preflight 共用）。**WorkBuddy / Windows**：拉数优先 **`scripts/query_market_facts.ps1`**（内置 `PYTHONUTF8=1`）；展示全量表 **`read` `cache/snapshot/markdown_summary.md`** 或 `scripts/dump_markdown_summary.ps1`。zip 更新后**删除** `cache/snapshot/*` 再 `--force-refresh`。旧 cache（`snapshot_cache_encoding_version` 低于 2 或 `markdown_mojibake`）自动失效。  
- **带方向问信息（非全量）**：`python3 skills/streamy-content-gen/scripts/query_direction_brief.py --direction '…'` → 读 cache + 方向过滤，**禁止**改贴全量 `markdown_summary`。  
- **开稿 preflight**：`preflight_topic.py --direction '…'` **cache-first**（与 QMF 共用失效逻辑：`db_last_ingested_at` 优先，6h 回退）；`--force-refresh` 经 QMF 刷新。**禁止**默认 `--allow-targeted-fetch`。
- **快照预热 cron（业务机）**：`scripts/setup_snapshot_warm_cron.sh --install` → 北京时间 **08:15 / 09:45 / 14:05 / 20:05** 执行 `scripts/warm_snapshot_cache.sh`（云 API + 本机 Router/Rewriter，写同一缓存）。开稿前若缓存过期，先 warm 或等 cron，勿用 legacy。  
...
- 用户确认选题后、进入 `outline_refining` 前，必须展示并落盘该候选方向的 `evidence_pack`：**优先** `draft_manager update --draft <DID> --apply-topic-choice <N>` 或 `scripts/stream_gen_workflow_helper.py apply-choice --draft <DID> --candidate-id <N>`（读 `topic_candidates.candidate_evidence_packs`，无需二次 preflight）；再确认 **稿件类型 + 风格（一次）**：`helper list-profile-options` 展示组合（如「A 大盘观点·老丁」），用户一次回复后 `helper bind-profile --draft <DID> --content-type market_view|… --style-id <UUID>`；**禁止只问风格、跳过稿件类型**；未落盘证据包、`content_type` 或 `style_id` 为空时不得生成大纲/逐字稿（工具层会返回 `CONTENT_TYPE_REQUIRED_BEFORE_OUTLINE` / `STYLE_ID_REQUIRED_BEFORE_OUTLINE`）。
- `outline.md` / `script.md` / `meta.json` 不得手写或用 edit/write 直接改；大纲与逐字稿必须用 `draft_manager update --stage ... --payload-file ...` 落盘。**禁止** `write`/`edit` 到 `/tmp/outline_<DID>.json` 或 `/tmp/script_<DID>.json`；payload 路径固定为 `drafts/active/default/<DID>/_scratch/outline_payload.json` 与 `…/_scratch/script_payload.json`（可由 `draft_manager schema --draft <DID>` 返回）。若怀疑已有稿被绕过，先跑 `draft_manager.py doctor --draft <DID> --json`；归档稿加 `--include-archive`。
...
### 快照预热 cron（迁移 / 新路径 · 情况 B）

**脚本在 `workspace-stream-gen/` 内；cron 在宿主机当前 Linux 用户的 `crontab`，不随目录复制。**

复制、clone 或 WorkBuddy 迁到**新路径**后，必须在新仓库根：

```bash
./scripts/setup_snapshot_warm_cron.sh --install
./scripts/warm_snapshot_cache.sh   # 建议立刻写 cache/snapshot/snapshot.json
```

**部署/迁移检查（Agent 执行）**：

1. `crontab -l` 是否含 `# BEGIN stream-gen snapshot warm`  
2. 其中 `warm_snapshot_cache.sh` 的绝对路径是否等于 **`$(pwd)/scripts/warm_snapshot_cache.sh`**（不等则 `--install` 重做）  
3. `cache/snapshot/snapshot.json` 是否存在（或刚 warm 成功）

完整 SOP、WorkBuddy 教练 Prompt：`docs/DEPLOY_WORKBUDDY.md` §3.1。  
**勿与** `finance-ingest-cloud` 的 `setup_monitor_cron.sh`（云库飞书告警）混淆。
