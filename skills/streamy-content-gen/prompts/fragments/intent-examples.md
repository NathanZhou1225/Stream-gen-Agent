# Intent Examples（按需读取）

本文件为 **`natural-language-intent.md` 瘦身前** 的完整长版：用户话术表、阶段推进长表、反例与兜底话术等。日常默认只读 **`prompts/fragments/intent-core.md`**（由主索引 `natural-language-intent.md` 引入）；遇歧义、消歧、长表查词时再打开本文件。

---

# 自然语言意图识别清单（v0.1.5 · 归档长版）
...
**带明确方向开稿的降耗入口（可选）**：
若用户已给出清晰方向并要求开稿，可用 helper 打包非决策段，减少工具往返：

```bash
python3 scripts/stream_gen_workflow_helper.py start-topic --direction "<用户原话方向>"
```

该命令只到 `topic_picking` 三候选落盘；返回候选后必须停下来等用户选 1/2/3，不得继续生成大纲或逐字稿。
...
| 用户说法（易误判） | 正确定位 | **禁止** |
|---|---|---|
| 「60 秒口播**怎么切入**」/「**怎么讲** 今天 A 股」/「给一条 **X 秒** 口播」 | 等同 **开新稿 + 还没选题**，走拉数（若含时效/行情）→ `topic_picking` **三候选落盘** → 用户选序号 → **方向证据包** → user-style → 大纲 | 在**未**经过选题、方向证据包、风格门禁前，用纯对话输出**带分秒/分镜的完整口播**（如 `[0-5s]…[56-60s]…`），即跳过选题与大纲 |
...
**Agent 动作**：`draft_manager.py finalize --draft <focus> --min-context-reset`（仅清理会话层历史，保留 drafts/memory）
...
### 2.0A `candidate_evidence_pack_gate` — 选定候选后的方向证据包
...
**禁止**：用户选了候选后，直接进入风格或大纲而不展示该候选方向证据包；禁止把其它无关 D1/D2 单条详情当作所选方向详情。
...
- ❌ 为了省 token 把所有分析段统一写成 `role: "host"`。分析段仍须使用真实 role，并补 `claim_kind` / `evidence_source_*`。
- ❌ 为了省 token 刻意绕过 `user_style_context`。已绑定 `style_id` 时工具会自动注入；有 `user_style_context` 就必须补 `production_style_adaptation`。
...
- **纯拉数命令**：优先 **`query_market_facts.py --summary-only`**（默认轻输出：保留 `markdown_summary` + 精简 `meta/errors`，减少会话 token；仍与 `ingest.py` 同源）；直接调 `ingest.py` 仅用于调试。若需完整 JSON 可显式加 `--full`。脚本**不会**写入 `meta.websearch_required` / `meta.websearch_gaps`，也**不会**调用 `tavily-search`。若用户追问某缺口（如北向为 0、社媒为空），可由 Agent **自愿**使用会话内的 WebSearch 在对话中补充说明，且**不得**改写或声称替换了 `markdown_summary` / `sections` 中的 API 数值。  
