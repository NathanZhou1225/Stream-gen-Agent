...
✅ **步骤3（方向证据包闸 · 已与 preflight 合并）**：用户选择候选后，**优先** `draft_manager update --draft <DID> --apply-topic-choice <N>`（一次写入 `chosen` + `candidate_evidence_pack.json`，读 `topic_candidates.candidate_evidence_packs[N]`，**无需**二次 preflight、**无需**临时 evidence JSON）。或 `helper apply-choice --draft <DID> --candidate-id <N>`（同上；旧稿无内嵌包时自动 legacy 回退）。证据包必须围绕已选方向，不得展示无关 D1/D2 单条详情。
...
- 用户选定后，可用 `scripts/stream_gen_workflow_helper.py apply-choice --draft <DID> --candidate-id <N>`（默认 1 次 `--apply-topic-choice`，无 `/tmp` 证据 JSON）；旧稿用 `--force-preflight`。返回后必须展示证据包并等用户确认进入 user-style。
- 风格：`list-styles`（`style_cli list --with-context`）→ 用户选 → `bind-style --draft <DID> --style-id <UUID>`（或分步 `--set-style-id` + `get-context`）。
- 逐字稿：生成前 `prevalidate-script --payload-file <json>`；落盘前 `validate-script`（`--validate-only`）。
